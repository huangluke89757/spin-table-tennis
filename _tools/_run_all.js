...
/* 手机横屏可用性（本次缺陷的专项回归）。
 * 为什么单独立一关：上面两关都覆盖不到「手机横屏」这个形状 ——
 *   · _touch.js 是纯函数桩，量不到「按钮是否真的在视口内」；
 *   · _touch_browser.js 原本只有平板横屏（900 宽 ≥ 旧门槛 768）与手机竖屏，
 *     恰好绕过了「手机横屏被误拦」这条路径，所以缺陷一直绿着。
 * 本关用真机 CSS 视口（568×320 ~ 832×380）验三层：门控放行 → 开始按钮在屏内
 * → 触摸能真的回球，并用竖屏视口反向确认拦截没被改坏。 */
allOk = run("12b/14 手机横屏可用性（门控 · 按钮可达 · 触摸可玩）", "_mobile_land_verify.js",
            { NODE_PATH: WS }, 12) && allOk;
/* 结算页左右分栏 + 本轮四项改造（2026-09-25 移动端走查）。
 * 为什么单独占一关：主症「再来一局」被挤出首屏是**几何**问题（手机横屏可视高仅
 * 320~430px，而结算内容 700~900px），_touch.js 的纯函数桩量不到「按钮在不在视口内」；
 * 并列得分被排成 1、2 名则是**逻辑**问题，只有真渲染出榜单才看得见。 */
allOk = run("12c/14 结算页分栏与四项改造（按钮首屏 · 并列名次 · 练习对照卡）", "_end_layout_verify.js",
            { NODE_PATH: WS }, 12) && allOk;
