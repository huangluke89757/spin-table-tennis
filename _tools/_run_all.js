...
let allOk = true;
allOk = run("1/11 语法检查", "_syntax.js") && allOk;
allOk = run("2/11 物理内核回归（含求解器性能）", "_phys_test.js", null, 6) && allOk;
/* 物理数值快照：优化热路径后必须逐位一致。
 * 为什么单独一关：K 系数是离线标定到 732/732 的，浮点末位差异在极端弹道上
 * 可能把"擦网上台"翻成"下网"。_phys_test 只覆盖代表性用例，这一关跑 4320 条弹道做逐位比对。 */
allOk = run("3/11 物理内核数值一致性（新旧实现逐位比对）", "_phys_perf.js", null, 5, ["--compare"]) && allOk;
allOk = run("4/11 完整冒烟（含音效链路）", "_smoke.js", null, 22) && allOk;
allOk = run("5/11 真浏览器截图", "_shot.js", { NODE_PATH: WS }) && allOk;
allOk = run("6/11 海报 / 抓帧 / 残影 / 二维码", "_poster_shot.js", { NODE_PATH: WS }) && allOk;
allOk = runPy("7/11 二维码独立交叉验证", "_qr_verify.py") && allOk;
/* 底部辅助面板布局：重叠是「盒子在屏幕上的位置」问题，Node 桩量不出来，
 * 必须真浏览器量 getBoundingClientRect 两两求交。 */
allOk = run("8/11 辅助面板布局（多尺寸 · 不重叠 · 拍面栏同收放）", "_aux_shot.js", { NODE_PATH: WS }, 10) && allOk;
/* 观众欢呼声学验证：把真实 game.js 音效代码离线渲染成 PCM 做 FFT。
 * 为什么必须做：代码结构对（"创建了振荡器"）不等于听感对——
 * 用户反馈"轰轰轰"必须用频谱客观证明真的变了，而不是靠自我感觉。 */
allOk = run("9/11 观众欢呼声学验证（离线渲染 + FFT）", "_audio_spectrum.js", { NODE_PATH: WS }, 4) && allOk;
allOk = runPy("10/11 欢呼声频谱分析（逐层定位 + 新旧对照）", "_audio_spectrum.py") && allOk;
...
if (process.argv.indexOf("--live") >= 0) {
  allOk = run("11/11 线上端到端验证（访问线上地址）", "_live_verify.js", { NODE_PATH: WS }, 8) && allOk;
} else {
